...
The official `wordpress/plugin-check-action@v1` was added to CI in v0.11.1 and properly scoped in v0.11.2. Below are the items it (or the future human reviewer) will still flag and that need a human decision :

- [ ] **🚨 BLOCKER — Rename the plugin** : the WordPress trademark guideline forbids the word "WordPress" in both the plugin name AND the slug. Both `"AB Testing WordPress"` and the slug `ab-testing-wordpress` will be rejected at review. Pick a new name (e.g. `"AB Testing for WP"`, `"Lozit A/B Testing"`, `"Conversion AB Tests"`, etc.) and a matching slug. Affects : plugin header, `readme.txt`, `composer.json`, `release.yml` build path (`build/<slug>/`), `.wp-env.json`, the demo seed script, the `tools/` script, every audit report referencing the slug, GitHub repo name (optional but consistent). **Plan a v0.12.0 dedicated to the rename — it's a coordinated multi-file change**.
